...
| 更好的错误定位 | 已覆盖：Bison 错误包含当前 token 行列 |
| 续编译、简单纠错 | 部分覆盖：语句级错误恢复，错误语句不生成 TAC，后续语句继续翻译 |
...
实验指导书扩展要求中提出了错误定位、续编译和简单错误处理。当前 Bison
路径实现的是语句级错误恢复。
...
- 以语句边界为恢复单位；
- 主要同步点是分号、`end` 和 EOF；
- 发生语法错误的语句构造成 `TacError` 节点；
- `TacEmitter` 遇到 `TacError` 时跳过该语句，不生成 TAC；
- 后续合法语句继续参与 TAC 生成。
...
输出会先报告第 1 行分号处的语法错误，然后继续生成第二条赋值语句的
TAC：
...
该能力由 `lab3_tac_error_recovery.*` fixture 覆盖。当前实现不做复杂
自动纠错，只保证错误语句跳过和后续语句续编译。
...
- Bison 实验三路径当前只做语句级错误恢复，不做复杂自动纠错；
